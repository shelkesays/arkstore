...
Rules the Postgres loader applies (implementation notes for §5.4–5.6):

- "Empty" ignores **extension-owned** objects: an installed extension belongs
  to its extension, which the archive recreates with `CREATE EXTENSION IF NOT
  EXISTS`. Everything else in a non-system schema — any relation kind,
...
- Load phases: structure for every non-trigger object in load-plan order
  (cyclic objects last — their foreign keys are post-data anyway), then every
  data file by `COPY … FROM STDIN`, then **triggers** (created only after the
...
optional `port`, `tls` / `tls_ca_file` (same values and meaning as a source's),
mongo `auth_db`, and `ephemeral` (§12); file `name`, `path`. Missing file is
fine; targets may be inline (`restore.target`) or fully overridden by CLI/env
(§5.2).
