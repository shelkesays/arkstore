...
`--features postgres` or download the full release."* — mirroring AutoVault's optional-extras model
(`autovault[postgre,archive]`) but resolved at compile time into one artifact.
...
# e.g. archive/supersetdb/logs/logs.2026-04.parquet
```
...
Where AutoVault processed sources one at a time, Arkstore runs them in parallel — but the model
